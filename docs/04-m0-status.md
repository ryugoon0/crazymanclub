...
## Gate 판정 기준 결정 (해결됨)
사용자 결정: **A — 기준을 `low1_wall_fps`로 교체.** 결정 M-2로 `docs/03-m0-decisions.md`에 기록. `tools/bench_runner.gd`의 `gate_ok`와 리포트 문구를 wall-clock 계열 기준으로 수정 완료. 200마리 PASS는 그대로 유효(wall 1% low 150.6, 기준 45의 3.3배).
